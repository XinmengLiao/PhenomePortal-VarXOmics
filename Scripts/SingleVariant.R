...
# pQTL scores only come from Open Target Genetics (conducted in python)

cat("\n1. Adding pQTL Evidence\n")
...
if(nrow(pqtl.df)> 0){
  write.table(pqtl.df,file = gsub(".txt",".pqtl-epigraphdb.txt", input_file),quote = F,sep = "\t",row.names = F)
}else{
  write.table(data.frame("Note" = "No results or data found. " ),
              file = gsub(".txt",".pqtl-epigraphdb.txt.txt", input_file), quote = F,sep = "\t",row.names = F)
}


## ---- eqtl-mr (EpigraphDB) ----
cat("\n2. Adding eQTL MR Evidence\n")
...
if(grepl("^rs", rsid)){
  ...
  if( !is.null(results)){
    ...
    write.table(data.frame("Note" = "No results or data found. " ),
                gsub(".txt",".snpmr.txt", input_file), quote = F,sep = "\t",row.names = F)
  }
}
...
if (nrow(user.lol.data) > 0) {
  ...
  jsonlite::write_json(chart$x, path = paste0(output_dir, "/Lollipop.json"), pretty = TRUE, auto_unbox = TRUE)
}
