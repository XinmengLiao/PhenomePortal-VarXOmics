...
EXOMISER_CONFIG="$OUTPUT_DIR/$INPUT_SAMPLE-Exomiser-genome-REMM.yml"

if [[ ! -f "$SCRIPTS_DIR/sample-Exomiser-genome-REMM.yml" ]]; then
    echo "Error: Template Exomiser config file not found: $SCRIPTS_DIR/sample-Exomiser-genome-REMM.yml"
    exit 1
fi

cp "$SCRIPTS_DIR/sample-Exomiser-genome-REMM.yml" "$EXOMISER_CONFIG"

echo "[DEBUG] Editing config file: $EXOMISER_CONFIG"

# Step 3.2: Configure gene panel filter
echo "$(date): Configuring gene panel filter..."
if [[ -f "$OUTPUT_DIR/${INPUT_SAMPLE}_genes.txt" ]]; then
    GENES_LIST=$(cat "$OUTPUT_DIR/${INPUT_SAMPLE}_genes.txt")
    sed -i "s|genePanelFilter: {geneSymbols: \[\]}|genePanelFilter: {geneSymbols: [$GENES_LIST]}|" "$EXOMISER_CONFIG"
else
    echo "Warning: Gene list file not found: $OUTPUT_DIR/${INPUT_SAMPLE}_nodup_genes.txt"
fi
...
if ! java -jar "$EXOMISER_DIR/exomiser-cli-14.0.0.jar" \
    ...
    --output-filename "${INPUT_SAMPLE}_Exomiseri_REMM" \
